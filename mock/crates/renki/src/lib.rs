...
fn outcome(tool: &Tool, raw: &[OsString]) -> Result<(), String> {
    ...
    // The settings table is checked here rather than in `defect`, since a
    // kind's parser is not const: a key that names no variable, a key declared
    // twice, or a default the row's own kind refuses.
    if let notko::Maybe::Is(bad) = renki_config::Declared::defect(tool.settings) {
        return Err(format!(
            "this launcher's settings table is not usable: {bad}"
        ));
    }
...
fn dispatch(tool: &Tool, args: &[OsString]) -> Result<(), String> {
    ...
    // `--cfg key=value` is the launcher's too, and the tool's own variables
    // are read with it, so the settings can be resolved once below and the
    // engine never sees the flag.
    let (cli, args) = config::Cli::take(tool, args)?;
    ...
    // The same for the configuration: the engine is what is being configured
    // and may not build, so its settings are answered without it. A repository
    // is not needed for the person's file, and is used for the repository's
    // where one is found.
    if config::query::is_the_config_query(tool, args) {
    ...
    // The settings, resolved once: the flag, the variable, the repository's
    // file, the person's file, the default. The engine gets the result as its
    // environment and never has two answers to choose from.
    let user_file = config::user_file(tool)?;
    let repo_file = located.as_ref().map(|l| l.config_path.clone());
    let texts = config::read_texts(&user_file, repo_file.as_deref())?;
    let settings = config::resolve_all(tool, &cli, &texts)?;
    let envs = config::engine_env(tool, &user_file, &settings);
    ...
    if let Some(raw) = engine_override {
        ...
        let extra = tool
            ...
            .map(|f| f(&source))
            .unwrap_or_default();
        return cache::exec_engine(tool, &bin, &workdir, &extra, args, &envs).map(|_never| ());
    }
    ...
    let extra = tool
        ...
        .map(|f| f(&resolved))
        .unwrap_or_default();
    cache::exec_engine(tool, &bin, &workdir, &extra, args, &envs).map(|_never| ())
}
