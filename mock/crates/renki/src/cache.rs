...
// lint:allow(trait-first-signatures) reason: the argv handed to `exec`, at the launcher's std boundary. FIXME: an iterator once the callers take one.
...
pub(crate) fn exec_engine(
    ...
    let err = Command::new(bin)
        .arg0(&argv[0])
        .args(&argv[1 ..])
        .envs(envs.iter().map(|(k, v)| (k.as_str(), v.as_os_str())))
        .exec();
