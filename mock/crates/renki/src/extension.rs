...
pub trait Backend {
    ...
    /// no reason.
    // lint:allow(no-vec-in-trait-sig, trait-first-signatures) reason: a backend's fingerprint is text it owns, at the launcher's std boundary. FIXME: a sink-shaped fingerprint once the stack's string type is taken.
    ...
    // lint:allow(no-vec-in-trait-sig, trait-first-signatures) reason: the error is a message the operator reads, at the launcher's std boundary. FIXME: a typed refusal once the stack's string type is taken.
...
impl Registry {
    ...
    // lint:allow(trait-first-signatures) reason: the backend names for a diagnostic, at the launcher's std boundary. FIXME: an iterator once the callers take one.
...
// lint:allow(trait-first-signatures) reason: an argument list handed to `Command::args`, at the launcher's std boundary. FIXME: an iterator once the callers take one.
    ...
    fn materialise(plan: &Self::Plan, into: &Path) -> Result<(), String> {
        ...
        let run = |attempt: &[String]| -> Result<Attempt, String> {
            ...
            Ok(if !status.success() {
                Attempt::Failed
            } else if into.join("bin").join(&plan.bin).is_file() {
                Attempt::Built
            } else {
                Attempt::NoBinary
            })
        };
        install_with_retry(&plan.attempts, &plan.crate_name, run, || {
...
// lint:allow(trait-first-signatures) reason: the removed keys for logging, owned once per pass at the launcher's std boundary. FIXME: an iterator once the callers take one.
